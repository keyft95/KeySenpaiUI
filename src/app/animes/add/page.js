...
export default function AddAnime(){
    ...
    const handleChange = (event) => {
        ...
        setNewAnime((values) => {return {...values, [name]: value}})  
    }
    ...
    const handledSubmit = (event) => {
        ...
        .then((apiData) => {
            console.log(apiData);
            alert("Se ha creado el Anime")
            router.push("/animes")//nombre de la carpeta donde quiero redireccionar
        })
    }
    return(
        <>
            <h1 className="m-5 text-2xl text-center text-rose-300">Agregar Anime</h1>
            <form onSubmit={handledSubmit} className="flex flex-col px-5 gap-3">
                ...
                <select name="estadoAnime" onChange={handleChange} className="py-3 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                    <option value="">Estado</option>
                    <option value="ENPROGRESO">En Progreso</option>
                    <option value="FINALIZADO">Finalizado</option>
                    <option value="HIATUS">Hiatus</option>
                </select>
                <select name="tipoAnime" onChange={handleChange} className="py-3 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
